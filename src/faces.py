...
class VideoPlayer(QtWidgets.QWidget):
    ...
    def __init__(self, width=640, height=480, fps=30):
        ...
        self.Start_button = QtWidgets.QPushButton("Informatin")
        ...
        QtCore.QObject.connect(self.Start_button, QtCore.SIGNAL("clicked()"), self.information)
                ...
                for adress_dataset_from_face, dirs, files_dataset_from_face in os.walk("dataset_from_face"):
                    for file_from_face in files_dataset_from_face:
                        ...
                        if face_verify(img_1 = path_img_1, img_2 = str(full)) == True:
                            ...
                            new = text+data
                            if not os.path.exists(f"information"):
                                os.mkdir(f"information")

                                if not os.path.exists(f"information/information{filename1}"):
                                    os.mkdir(f"information/information{filename1}")

                                    if not os.path.exists(f"information/information{filename1}/{filename1}{datanow}"):
                                        os.mkdir(f"information/information{filename1}/{filename1}{datanow}")

                                        my_file = open(f"information/information{filename1}/{filename1}{datanow}/{filename1}{datanow}.txt", "w")
                                        my_file.write(new)
                                        my_file.close()
...
def face_analyze(img_path, path_analiz):
    try:
        result_dict = DeepFace.analyze(img_path=img_path, actions=['age', 'gender', 'race'])
        ...
        with open(f'images/{path_analiz}/group.txt', 'r') as fr, open(f'images/{path_analiz}/face_analyze.txt', 'w') as f:
            sys.stdout = f  # Change the standard output to the file we created.
            group = fr.read()
